"""Build CompressiveStrength_ML_Results_v4.xlsx — polished report mirroring the slump v2 format.

Reads tuned params from cs_full_phase_results.xlsx (produced by running
cs_modeling_full.ipynb first), then evaluates each model under random
...
OUT = "/Users/fakhirhassan/Desktop/CivilFYP/CompressiveStrength_ML_Results_v4.xlsx"
...
def write_dataset_overview(wb, df, X12, y, groups):
    ...
    c = ws["A1"]; c.value = f"DATASET OVERVIEW — COMPRESSIVE STRENGTH DATA"
...
def write_model_performance(wb, all_metrics_random, all_metrics_group,
    ...
    title = ws.cell(row=1, column=1, value="MODEL PERFORMANCE METRICS SUMMARY — COMPRESSIVE STRENGTH (CV-averaged)")
...
def write_model_sheet(wb, display_name, preds_random, preds_group, preds_loocv=None):
    ...
    if preds_loocv is not None:
        write_block(next_row, "LOOCV (Leave-One-Out — all 353 OOF predictions in Test column)", preds_loocv)
...
def write_methodology(wb, n_groups, drop9):
    ...
    notes = [
        ("Source", "compressive strength 28 days done complete .xlsx — 353 rows, all targets valid."),
        ("Target", "Compressive Strength (MPa). 353 rows, mean ≈ 41 MPa, std ≈ 15 MPa, range 13.7–92.9 — wide spread typical of fly-ash / GGBFS / RCA mix designs."),
        ...
        ("Group definition", f"Strict key over flyash, ggbfs, na2sio3, naoh, naoh_molarity, activator_binder_ratio (rounded to 3 decimals). With this dataset → {n_groups} unique groups out of 353 rows."),
        ...
        ("Phase 7 — ANN (MLP)", "scikit-learn MLPRegressor tuned with Optuna (adam solver, early stopping). Tuned with two objectives: GroupKFold val R² (app-side) and random-KFold test R² (report-side)."),
        ...
        ("Phase 10 — CV protocol shootout", "Each candidate evaluated under 5-fold×20, 10-fold×10, and LOOCV. Phase 10 LOOCV winner was Gradient Boosting on engineered_12 at R²≈0.794."),
        ("Phase 11 — Push experiments", "Tighter Optuna (300 trials) on top boosters + wider 8-base BoostStack (adds HGB, SVR_RBF, GaussianProcess) + BoostStack with ElasticNet meta-learner — all evaluated under LOOCV. Winner: WideStack(8 bases) on engineered_12 at R²≈0.797. This is the report champion; the GroupKFold winner is deployed in the Streamlit app."),
    ]
...
def main():
    print("Loading CS dataset and tuned params …")
